...
function renderProductCard(product, delay = 0) {
  ...
  const isOut      = product.available !== false && product.stock !== null && product.stock !== undefined && product.stock === 0;
  const lowStock   = product.stock !== null && product.stock !== undefined && product.stock > 0 && product.stock <= 5;
  const waNotifyText = encodeURIComponent('Hi SissyTrends! I am interested in ' + product.name + ' (ID: ' + (product.productId||'-') + '). Please notify me when it is back in stock.');
  const notifyClick  = '(function(){var b=(location.hostname===\'localhost\'||location.hostname===\'127.0.0.1\')?\' http://localhost:5000\':\'\'  ;fetch(b+\'/api/products/' + product.id + '/notify\',{method:\'POST\'}).catch(function(){});})();';

  return `
    <div class="product-card reveal" style="transition-delay:${delay}s;position:relative${(isSoldOut||isOut)?';opacity:0.85':''}"
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          ...
          <a href="https://wa.me/919344182144?text=${waNotifyText}" target="_blank"
            onclick="event.stopPropagation();${notifyClick}"
